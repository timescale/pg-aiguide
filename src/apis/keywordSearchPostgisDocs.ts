...
export const keywordSearchPostgisDocsFactory: ApiFactory<
  ServerContext,
  typeof inputSchema,
  typeof outputSchema,
  z.infer<(typeof outputSchema)['results']>
> = ({ pgPool, schema }) => ({
  name: 'keyword_search_postgis_docs',
  method: 'get',
  route: '/keyword-search/postgis-docs',
  config: {
    title: 'Keyword Search of PostGIS Documentation',
    description:
      'This retrieves relevancy ranked documentation entries based on a set of keywords, using a bm25 search. The content covers PostGIS spatial database extension topics including geometry types, spatial functions, indexing, and GIS operations.',
    inputSchema,
    outputSchema,
  },
...
SELECT
  ...
  content,
  metadata::text,
  -(content <@> to_bm25query($1, 'docs.postgis_chunks_content_idx')) as score
 FROM ${schema}.postgis_chunks
 ORDER BY content <@> to_bm25query($1, 'docs.postgis_chunks_content_idx')
 LIMIT $2
`,
      [keywords, limit || 10],
    );
